...
STATICFILES_DIRS = [
    BASE_DIR / 'static',
    BASE_DIR / 'backend' / 'static',  # include built frontend output so collectstatic picks it up
]
...
CORS_ALLOWED_ORIGINS = os.getenv(
    ...
    'http://localhost:5173,http://localhost:5174,https://cohort26-ten.vercel.app'
).split(',')
# Temporarily allow all origins to debug CORS issues
CORS_ALLOW_ALL_ORIGINS = True  # Force allow all for debugging
CORS_ALLOW_CREDENTIALS = True
CORS_ALLOW_METHODS = ['DELETE', 'GET', 'OPTIONS', 'PATCH', 'POST', 'PUT']
CORS_ALLOW_HEADERS = ['accept', 'accept-encoding', 'authorization', 'content-type', 'dnt', 'origin', 'user-agent', 'x-csrftoken', 'x-requested-with']

# CSRF Trusted Origins (for Django's CSRF protection)
